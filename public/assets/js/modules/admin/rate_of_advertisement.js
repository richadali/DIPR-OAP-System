    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response, function (indexInArray, valueOfElement) {
                var categoryName = valueOfElement.ad_category
                    ? valueOfElement.ad_category.category_name
                    : "-";
                table +=
                    "<tr>" +
                    ...
                    "</td>" +
                    '<td class="text-center">' +
                    valueOfElement.advertisement_type.name +
                    "</td>" +
                    '<td class="text-center">' +
                    categoryName +
                    " </td>" +
                    '<td class="text-center">' +
                    valueOfElement.amount +
                    "</td>";

                table +=
                    '<td class="text-center"><span style="color:darkblue;" data-id="' +
                    valueOfElement.id +
                    '"class="icon ri-edit-2-fill rate-edit"></span> &nbsp; &nbsp;<span style="color:red;" data-id="' +
                    valueOfElement.id +
                    '" class="icon ri-chat-delete-fill rate-delete"></span> </td>';
                ("</tr>");
            });
    ...
    $(document).on("click", ".rate-edit", function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#id").val(data[0]["id"]);
                $("#ad_category")
                    .val(data[0]["ad_category_id"])
                    .trigger("change")
                    .attr("readonly", true);
                $("#ad_category_hidden").val(data[0]["ad_category_id"]);
                $("#advertisementType")
                    .val(data[0]["advertisement_type_id"])
                    .trigger("change")
                    .attr("readonly", true);
                $("#advertisement_type_hidden").val(
                    data[0]["advertisement_type_id"]
                );
