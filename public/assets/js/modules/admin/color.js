    ...
    $.ajax({
      ...
      success: function (response) {
          ...
          $.each(response, function (indexInArray, valueOfElement) { 
               ...
               '<td class="text-center">'+ valueOfElement.color_name+ '</td>';
               table += '<td class="text-center"><span style="color:darkblue;" data-id="'+ valueOfElement.id+'"class="icon ri-edit-2-fill subject-edit"></span> &nbsp; &nbsp;<span style="color:red;" data-id="'+ valueOfElement.id+'" class="icon ri-chat-delete-fill subject-delete"></span> </td>'
               '</tr>';
          });
      ...
      $(document).on('click','.subject-edit',function (e) { 
          e.preventDefault();
          var id = $(this).data('id');
          var value;
          $.ajax({
            ...
            success:function(data){
              $('#color-form').trigger("reset");
              $("#id").val(data[0]['id']);
              $("#color").val(data[0]['color_name']);
              $("#users-tab").tab('show');
            }
          });
        });
  
        $(document).on('click','.color-delete',function (e) { 
          e.preventDefault();
          var id = $(this).data('id');
          $.ajax({
            type: "POST",
            url: "/color-delete-data",
            data: {id},
            cache:false,
            success:function(data){
              if(data['flag']=='Y'){
